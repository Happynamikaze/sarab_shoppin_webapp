...
const Footer = ({ likedCount = 0, dislikedCount = 0, cartedCount = 0 }) => {
  return (
    ...
    <Link to="/liked">
      ...
      <Link to="/disliked">
        <button className="bg-red-500 text-white px-4 py-2 mx-4 rounded-lg flex justify-center items-center ">
          <img src={Dislike} alt="Dislike" className="w-4 h-4 me-2" />
          <p>{dislikedCount}</p>
        </button>
      </Link>
      <Link to="/cart">
        <button className="bg-yellow-500 text-white px-4 py-2 rounded-lg flex justify-center items-center ">
